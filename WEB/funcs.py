...
def makeWeek(initDay):
    ...
    date = datetime.datetime(_year,_month,_day)

    _dayOfWeek = date.isoweekday()

    date = date - datetime.timedelta(days = _dayOfWeek - 1)
    ...
    for i in range(7):
        ...
        for j in range(8,16):
            week[i].append( (datetime.datetime(date.year,date.month,date.day,j) , datetime.datetime(date.year,date.month,date.day,j + 1) , vanilla_activity ) )
...
def resetWeek(week):
    ...
    date,_,_ = week[0][0]
    return makeWeek(f'{date.year}-{date.month}-{date.day}')


def getDay(week,day):
    #day indexado em base 0
    # L = 0, M = 1 .... D=6
    activities = []
    for x in week[day]:
        _,_,_act = x
...
def getWebDay(activities):
    webActivities = []
    for b,e,act in activities:
        date = f'{b.day}-{b.month}-{b.year}'
        b_hour = f'{b.hour}:{b.minute}'
        e_hour = f'{e.hour}:{e.minute}'
        webActivities.append( (act.name ,date,b_hour,e_hour ))
    return webActivities


def exportWeek(week,bWeek,canLoad,free_Turns,real_free_Turns,activities,actNumb,exportName):
    ...
    data['canLoad'] = canLoad
    data['myWeek'] = bWeek
    data['actNumb'] = actNumb
    ...
    data['real_free_Turns'] = real_free_Turns

    for day in range(len(week)):
        
        for turn in week[day] :
            _begin , _end , _act = turn 
            data['week'][day].append( ( (_begin.year,_begin.month,_begin.day,_begin.hour,_begin.minute) ,(_end.year,_end.month,_end.day,_end.hour,_end.minute) , _act.name  ) )

    for day in range(len(free_Turns)):
        for turn in free_Turns[day] :
            data['free_Turns'][day].append((turn.year,turn.month,turn.day,turn.hour,turn.minute))

    with open(f'{exportName}.json','w') as file:
        json.dump(data,file,indent=4)

def loadWeek(importName):

    with open(importName) as file:
        data = json.load(file)

        week = [[] , [], [] , [] ,[ ] , [] ,[]]
        bWeek = data['myWeek']
        canLoad = data['canLoad']
        actNumb = data['actNumb']
        activities = data['activities']
        real_free_Turns = data['real_free_Turns']   
        free_Turns = [[] , [], [] , [] ,[ ] , [] ,[]]

        for i in range(7):
            for act in data['week'][str(i)]:
                _begin = act[0]
                _end = act[1]
                _act = act[2]
                myAct = Activity(None,None,None,_act)
                week[i].append( (  datetime.datetime(_begin[0],_begin[1],_begin[2],_begin[3],_begin[4]) , datetime.datetime(_end[0],_end[1],_end[2],_end[3],_end[4]) , myAct  ) )
        
        for i in range(7):
            for y,m,d,h,mi in data['free_Turns'][str(i)]:
                free_Turns[i].append(datetime.datetime(y,m,d,h,mi))

        return week,bWeek,canLoad,activities,actNumb,free_Turns,real_free_Turns

#my_week = makeWeek("12-10-2020")

#printWeek(my_week)
#print('*'*100)
#print_activity_list(getDay(my_week,0))
#loadFile('activities.json',my_week)
#printWeek(my_week)
#print_activity_list(getDay(my_week,4))
